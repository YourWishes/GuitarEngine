...
public class GUIBorder extends GUIObject {
    ...
    public void setWidth(int width) {
        double individual_width = TOP_LEFT.width;
        if(width < individual_width * 2) return;
        TOP_MIDDLE.width = BOTTOM_MIDDLE.width = CENTER.width = width-(individual_width*2.0);
        TOP_MIDDLE.x = BOTTOM_MIDDLE.x = CENTER.x = individual_width;
        TOP_RIGHT.x = MIDDLE_RIGHT.x = BOTTOM_RIGHT.x = TOP_MIDDLE.width + TOP_MIDDLE.x;
    }
    
    public void setHeight(int height) {
        double individual_height = TOP_LEFT.height;
        if(height < individual_height * 2) return;
        MIDDLE_LEFT.height = CENTER.height = MIDDLE_RIGHT.height = height-(individual_height*2.0);
